...
def screenshot_board(zoom_size: int, col_x_coords: dict, row_y_coords: dict):
    width = (zoom_size * 30) + 10
    height = (zoom_size * 16) + 10
...
def return_tile_type(zoom_size: int, col_x_coords: dict, row_y_coords: dict, screenshot, tile: tuple[int, int]) -> int:
...
def bfs(gameboard, col_x_coords, row_y_coords, tile, zoom_size, screenshot) -> None:
    ...
    while len(queue) > 0:
        ...
        cur_tile_type = return_tile_type(zoom_size, col_x_coords, row_y_coords, screenshot, cur_tile)
        if cur_tile_type <= 0:
...
def update_tiles_dev_mode(gameboard, col_x_coords, row_y_coords, tile, zoom_size, screenshot) -> None:
    col_num = tile[0]
    row_num = tile[1]
    val = return_tile_type(zoom_size, col_x_coords, row_y_coords, screenshot, tile)
    if val == 0:
        # if current tile is an open space, we use bfs to update all tiles in the surrounding area
        bfs(gameboard, col_x_coords, row_y_coords, tile, zoom_size, screenshot)
    elif val != -2:
...
def update_tiles(gameboard, col_x_coords, row_y_coords, tile, zoom_size, screenshot) -> int:
    col_num = tile[0]
    row_num = tile[1]
    val = return_tile_type(zoom_size, col_x_coords, row_y_coords, screenshot, tile)
    if val == 0:
...
def click_tile_and_update_board(gameboard, col_x_coords, row_y_coords, zoom_size, tile, screenshot) -> int:
    col_num = tile[0]
    row_num = tile[1]
    pyautogui.click(col_x_coords.get(col_num), row_y_coords.get(row_num))
    loss_status = update_tiles(gameboard, col_x_coords, row_y_coords, tile, zoom_size, screenshot)
    return loss_status
